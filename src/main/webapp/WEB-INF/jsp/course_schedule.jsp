            ...
            $('#table').bootstrapTable({
                ...
                columns: [
                    ...
                    },
                    {
                        field:'xx',title:'操作',
                        formatter : function(value, row, index) {
                            return "<a title='编辑' href='javascript:upd2(" + row.scheduleId + ", \"" + row.classroom.classroomName + "\", \"" + row.coach.coachName + "\", \"" + row.subject.subname + "\", \"" + row.startTime + "\", \"" + row.endTime + "\", " + row.courseStatus + ")'><i class='fa fa-edit'></i></a>" +
                                "<a title='删除' href='javascript:del2(" + row.scheduleId + ")'><i class='fa fa-trash'></i></a>  ";
                        }
...
<%--            <button onclick="search()" type="button" class="btn btn-default" >查询</button>--%>
            <button type="button" class="btn btn-default" style="float: right;" onclick="totian(event)"><span class="glyphicon glyphicon-plus"></span>安排新课程</button>
