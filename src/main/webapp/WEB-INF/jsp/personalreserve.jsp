...
<script>
    ...
    $(function() {
        ...
        // 页面加载完成后立即执行的代码
        var memberId = $('#memberId').val(); // 从隐藏的 input 字段获取 memberId

        // 修改此 AJAX 请求以包含 memberId，以便服务器知道需要返回哪个会员的预约信
        $.ajax({
                url: '${pageContext.request.contextPath}/reserve/query' + (memberId ? '?memberId=' + memberId : ''), // 添加 memberId 作为查询参数
                method: 'GET', // 修改为 GET 请求
            contentType: 'application/json', // 指定发送数据的格式
            success: function(data) {
                let tbody = $('#reservationsTable tbody');
                tbody.empty(); // 清空表格中现有的行
                data.rows.forEach(function(row) {
                    // 这里假设返回的数据格式没有变化
                    var dateString = new Date(row.date).toISOString().split('T')[0];
                    var currentDate = new Date().toISOString().split('T')[0]; // 获取当前日期的字符串表示
                    var actionButton = currentDate <= dateString ? '<button class="btn btn-danger btn-xs cancel-reservation" data-id="' + row.reservationId + '">取消预约</button>' : '<span class="badge badge-success">已完成</span>';
                    var trHTML = '<tr>' +
        ...
        function fetchMemberNames() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var sessionMemberId = $('#memberId').val(); // 从隐藏字段获取 session 中的 memberId
                    response.forEach(function(member) {
                        var isSelected = member.memberId == sessionMemberId; // 判断当前会员是否为 session 中的会员
                        memberSelect.append(new Option(member.memberName, member.memberId, isSelected, isSelected));
                        if (isSelected) {
                            memberSelect.prop('disabled', true); // 如果选中，则禁用选择框
                        }
                    });
                },
                error: function(error) {
