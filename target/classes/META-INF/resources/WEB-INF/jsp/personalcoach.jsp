    ...
    <script>
        $(function () {
            $(function () {
                $('#table').bootstrapTable({
                    url: '${pageContext.request.contextPath}/coach/personalQuery',
                    method: 'post',
                    contentType: "application/x-www-form-urlencoded",
                    queryParams: function(params) {
                        return {
                            coachId: $("#coachId").val() // 只发送后端需要的coachId参数
                        };
                    },
                    ...
                    columns:[{ checkbox:true
                        ...
                        },
                        {
                            field:'xx',title:'操作',
                            formatter : function(value, row, index) {
                                return "<a title='编辑' href='javascript:upd1(" + row.coachId +")'><i class='fa fa-edit'></i></a>" ;

                            }

                        }
                    ],
                    // 其他 bootstrapTable 配置...
                    queryParamsType: '',
                    height: 360,
                    pageList: [5, 10, 15],
                    pageNumber: 1,
                    pageSize: 5,
                    pagination: true,
                    sidePagination: 'client', // 注意这里要改为client，因为数据是前端处理
                });
            });
        });
       ...
       function upd1(id){
            ...
            $.post('${pageContext.request.contextPath}/coach/cha',{'id':id},function(data){
                ...
                // 禁用开始上班时间、教练状态、教龄、薪资的编辑
                $('#xgdate').prop('disabled', true);
                $('#xgstatic').prop('disabled', true);
                $('#xgjl').prop('disabled', true);
                $('#xgxz').prop('disabled', true);
